...
// Do not use this class for now. Use a console generated key.
/**
 *
 *
 * @author developer
 */
public class HSMGenerateSymmetricKey extends HSMMessageBuilder {

    private final String GENERATE_KEY_MODE = "0";
     ...
     *
     */
    public VariantKeyType variantKeyType;
     ...
     *
     */
    public HSMGenerateSymmetricKey() {
        command.generateSymmetricKey();

        lmkKeyScheme = new LMKKeyScheme();
        variantKeyType = new VariantKeyType();
        keyExportability = new KeyExportability();
        keyUsage = new KeyUsage();
        keyModeOfUse = new KeyModeOfUse();

        addParameter(new MiscParameter(GENERATE_KEY_MODE, "Mode (Generate Key)"));
        addParameter(variantKeyType);
        addParameter(lmkKeyScheme);
        addParameter(new MiscParameter(VARIANT_DELIMITER, "Variant Delimiter"));
        addParameter(keyUsage);
        addParameter(keyModeOfUse);
        addParameter(new MiscParameter(VARIANT_VERSION_NUMBER, "Variant Version Number"));
        addParameter(keyExportability);
        addParameter(new MiscParameter(OPTIONAL_BLOCKS, "Number of Optional Blocks"));
    }
    ...
    public void defaultSettings() {
        variantKeyType.unidirectionalBDK();
        lmkKeyScheme.thalesVariantTrippleLengthLMKScheme();
        keyUsage.unidirectionalBDK();
