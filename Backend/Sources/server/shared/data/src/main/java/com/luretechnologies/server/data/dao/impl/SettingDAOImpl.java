...
public class SettingDAOImpl extends BaseDAOImpl<Setting, Long> implements SettingDAO {
    ...
    @Override
    public List<Setting> list(int firstResult, int lastResult) throws PersistenceException {
        return query(criteriaQueryComplex()).setFirstResult(firstResult).setMaxResults(lastResult).getResultList();
    }

    @Override
    public Setting findByName(com.luretechnologies.common.enums.SettingEnum name) throws PersistenceException {
        try {
            return (Setting) findByProperty("name", name).getSingleResult();
        } catch (NoResultException e) {
            LOGGER.info("Setting not found.", e);
            return null;
        }
    }

    @Override
    public List<Setting> list(SettingGroupEnum group, int firstResult, int lastResult) throws PersistenceException {

        CriteriaQuery<Setting> q = criteriaQuery();
        Root<Setting> root = getRoot(q);
        Predicate predicate = null;

        if (group != null) {
            Predicate groupPredicate = criteriaBuilder().equal(root.get("group"), group);
            predicate = criteriaBuilder().and(groupPredicate);
        }
