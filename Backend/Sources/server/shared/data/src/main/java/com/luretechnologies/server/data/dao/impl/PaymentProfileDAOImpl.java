 ...
 */
@Repository
public class PaymentProfileDAOImpl extends BaseDAOImpl<PaymentProfile, Long> implements PaymentProfileDAO {

    private static final Logger LOGGER = LoggerFactory.getLogger(PaymentProfileDAO.class);

    @Override
    public PaymentProfile findByName(String name) throws PersistenceException {
        try {
            return (PaymentProfile) findByProperty("name", name).getSingleResult();
        } catch (NoResultException e) {
            LOGGER.info("PaymentProfile not found. name: " + name, e);
            return null;
        }
    }

    @Override
    public List<PaymentProfile> list(int firstResult, int lastResult) throws PersistenceException {
        return query(criteriaQueryComplex()).setFirstResult(firstResult).setMaxResults(lastResult).getResultList();
    }

    @Override
    public List<PaymentProfile> list(String name, int firstResult, int lastResult) throws PersistenceException {

        CriteriaQuery<PaymentProfile> q = criteriaQuery();

        Root<PaymentProfile> root = getRoot(q);
    ...
    @Override
    public List<PaymentProfile> search(String filter, int firstResult, int lastResult) throws PersistenceException {
