 ...
 */
@Repository
public class TerminalProfileDAOImpl extends BaseDAOImpl<TerminalProfile, Long> implements TerminalProfileDAO {

    @Override
    public List<TerminalProfile> list(int firstResult, int lastResult) throws PersistenceException {
        return query(criteriaQueryComplex()).setFirstResult(firstResult).setMaxResults(lastResult).getResultList();
    }

    @Override
    public List<TerminalProfile> search(String filter, int firstResult, int lastResult) throws PersistenceException {

        CriteriaBuilder critBuilder = criteriaBuilder();
        CriteriaQuery<TerminalProfile> q = criteriaQuery();
        Root<TerminalProfile> root = getRoot(q);
