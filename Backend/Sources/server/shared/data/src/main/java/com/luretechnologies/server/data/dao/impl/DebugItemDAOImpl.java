 ...
 */
@Repository
public class DebugItemDAOImpl extends BaseDAOImpl<DebugItem, Long> implements DebugItemDAO {

    @Override
    public List<DebugItem> search(Long entityId, String filter, int firstResult, int lastResult, Date dateFrom, Date dateTo) throws PersistenceException {

        CriteriaQuery<DebugItem> cq = criteriaQuery();
        Root<DebugItem> root = getRoot(cq);
        ...
        if (filter != null && !filter.isEmpty()) {
            predicate.getExpressions().add(criteriaBuilder().or(
                    ...
                    criteriaBuilder().like(criteriaBuilder().upper((Expression) root.get("message")), "%" + filter.toUpperCase() + "%")));
        }
