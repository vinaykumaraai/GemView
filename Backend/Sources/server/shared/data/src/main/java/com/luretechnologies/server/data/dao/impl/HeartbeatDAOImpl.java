 ...
 */
@Repository
public class HeartbeatDAOImpl extends BaseDAOImpl<Heartbeat, Long> implements HeartbeatDAO {

    @Override
    public List<Heartbeat> search(Long entityId, String filter, int firstResult, int lastResult, Date dateFrom, Date dateTo) throws PersistenceException {

        CriteriaQuery<Heartbeat> cq = criteriaQuery();
        Root<Heartbeat> root = getRoot(cq);

        Predicate predicate = criteriaBuilder().conjunction();

        if (filter != null && !filter.isEmpty()) {
            predicate.getExpressions().add(criteriaBuilder().or(
                    criteriaBuilder().like(criteriaBuilder().upper((Expression) root.get("hwModel")), "%" + filter.toUpperCase() + "%"),
                    criteriaBuilder().like(criteriaBuilder().upper((Expression) root.get("ip")), "%" + filter.toUpperCase() + "%"),
