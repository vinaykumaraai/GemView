...
@Repository
public class EntityLevelDAOImpl extends BaseDAOImpl<EntityLevel, Long> implements EntityLevelDAO{
 
     @Override
    public List<EntityLevel> getEntityLevelList(List<Long> IDS)throws PersistenceException{
        List<EntityLevel> entityLevelList = new ArrayList<>();
        for(Long ID : IDS) {
            entityLevelList.add(getEntityManager().find(EntityLevel.class, ID));
        }
    ...
    @Override
    public EntityLevel getEntityLevelByID(Long ID) throws PersistenceException{
       return  getEntityManager().find(EntityLevel.class, ID);
