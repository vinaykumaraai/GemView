 ...
 *
 */
package com.luretechnologies.server.front.ams.conf.interceptor;
...
public class ExceptionHandlerAdvice {
    ...
    public ErrorResponse handleMalformedJwtException(MalformedJwtException ex) {
        return logErrorResponse(HttpStatus.UNAUTHORIZED, new ErrorResponse(Constants.CODE_INVALID_TOKEN, Messages.INVALID_TOKEN), ex);
    }
    ...
    public ErrorResponse handleExpiredJwtException(ExpiredJwtException ex) {
        return logErrorResponse(HttpStatus.UNAUTHORIZED, new ErrorResponse(Constants.CODE_SESSION_EXPIRED, Messages.EXPIRED_TOKEN), ex);
    }
    ...
    public ErrorResponse handleNotAuthorizedException(NotAuthorizedException ex) {
        if (!ex.getChallenges().isEmpty() && ex.getChallenges().get(0).equals(Constants.CODE_INVALID_USER)) {
            return logErrorResponse(HttpStatus.UNAUTHORIZED, new ErrorResponse(Constants.CODE_INVALID_USER, Messages.INVALID_LOGIN_CREDENTIALS), ex);
        }
    ...
    public ErrorResponse handleConstraintViolationException(ConstraintViolationException ex) {
        ...
        if (ex.getConstraintViolations().iterator().next().getExecutableParameters() == null) {
    ...
    public ErrorResponse handleMaxUploadSizeExceededException(MaxUploadSizeExceededException ex) {
        return logErrorResponse(HttpStatus.BAD_REQUEST, new ErrorResponse(Constants.CODE_MAX_UPLOAD_SIZE_EXCEEDED, ex.getCause().getMessage().toUpperCase()), ex);
    }
