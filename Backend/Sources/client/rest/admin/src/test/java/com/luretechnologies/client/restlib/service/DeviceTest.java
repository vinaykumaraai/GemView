/**
 * COPYRIGHT @ Lure Technologies, LLC.
 * ALL RIGHTS RESERVED
 *
 * Developed by Lure Technologies, LLC. (www.luretechnologies.com)
 *
 * Copyright in the whole and every part of this software program belongs to
 * Lure Technologies, LLC (“Lure”).  It may not be used, sold, licensed,
 * transferred, copied or reproduced in whole or in part in any manner or
 * form other than in accordance with and subject to the terms of a written
 * license from Lure or with the prior written consent of Lure or as
 * permitted by applicable law.
 *
 * This software program contains confidential and proprietary information and
 * must not be disclosed, in whole or in part, to any person or organization
 * without the prior written consent of Lure.  If you are neither the
 * intended recipient, nor an agent, employee, nor independent contractor
 * responsible for delivering this message to the intended recipient, you are
 * prohibited from copying, disclosing, distributing, disseminating, and/or
 * using the information in this email in any manner. If you have received
 * this message in error, please advise us immediately at
 * legal@luretechnologies.com by return email and then delete the message from your
 * computer and all other records (whether electronic, hard copy, or
 * otherwise).
 *
 * Any copies or reproductions of this software program (in whole or in part)
 * made by any method must also include a copy of this legend.
 *
 * Inquiries should be made to legal@luretechnologies.com
 *
 */
package com.luretechnologies.client.restlib.service;

import com.luretechnologies.client.restlib.Utils;
import com.luretechnologies.client.restlib.common.ApiException;
import com.luretechnologies.client.restlib.common.CommonConstants;
import com.luretechnologies.client.restlib.service.model.UserSession;
import static org.junit.Assert.assertTrue;
import static org.junit.Assert.fail;
import static org.junit.Assume.assumeNotNull;
import org.junit.BeforeClass;
import org.junit.FixMethodOrder;
import org.junit.runners.MethodSorters;

/**
 *
 */
@FixMethodOrder(MethodSorters.NAME_ASCENDING)
public class DeviceTest {

    private static RestClientService service;
    private static UserSession userSession;

    /**
     *
     */
    @BeforeClass
    public static void createService() {

        service = new RestClientService(Utils.serviceUrl + "/admin/api", Utils.serviceUrl + "/payment/api");

        assumeNotNull(service);

        try {
            userSession = service.getAuthApi().login(CommonConstants.testStandardUsername, CommonConstants.testStandardPassword);
            assertTrue("User logged in", userSession != null);
        } catch (ApiException ex) {
            fail(ex.getResponseBody());
        }

    }
//
//    @Test
//    public void createDevice() {
//        Device device = new Device();
//        device.setName("Testing Device");
//        device.setDescription("Testing Device");
//        device.setSerialNumber("SFC1988");
//        device.setActive(Boolean.TRUE);
//        try {
//            List<Terminal> terminals = service.listTerminal(1, 10);
//            if (!terminals.isEmpty()) {
//                device.setParent(terminals.get(0));
//            }
//            device = service.createDevice(device);
//            id = device.getEntityId();
//            assertThat(device, instanceOf(Device.class));
//            assertNotNull(device.getEntityId());
//        } catch (ApiException ex) {
//            fail(ex.getResponseBody());
//        }
//    }
//
//    @Test
//    public void editDevice() {
//        try {
////            List<Device> listDevices = service.listDevices();
////            assertNotNull(service.listDevices());
////            Device device = null;
////
////            for (Device listDevice : listDevices) {
////                if (listDevice.getSerialNumber().equals("SFC1988")) {
////                    device = listDevice;
////                    break;
////                }
////            }
//            if (id != null) {
//                Device device = service.getDevice(id);
//
//                if (device != null) {
//                    assertEquals("SFC1988", device.getSerialNumber());
//                    device.setDescription("UPDATED");
//
//                    device = service.updateDevice(device.getEntityId(), device);
//                    assertEquals("UPDATED", device.getDescription());
//                }
//            }
//        } catch (ApiException ex) {
//            fail(ex.getResponseBody());
//        }
//    }
//
//    @Test
//    public void getDevice() {
//        try {
//            if (id != null) {
//                Device device = service.getDevice(id);
//                assertNotNull(device);
//            }
////            List<Device> listDevices = service.listDevices();
////            assertNotNull(service.listDevices());
////            for (Device listDevice : listDevices) {
////                if (listDevice.getSerialNumber().equals("SFC1988")) {
////                    Device device = listDevice;
////                    assertEquals("SFC1988", device.getSerialNumber());
////                    break;
////                }
////            }
//        } catch (ApiException ex) {
//            fail(ex.getResponseBody());
//        }
//    }
//
//    @Test
//    public void listDevices() {
//        try {
//            assertNotNull(service.listDevices(1, 50));
//        } catch (ApiException ex) {
//            fail(ex.getResponseBody());
//        }
//    }
//
//    @Test
//    public void findDevices() {
//        try {
//            List<Device> devices = service.searchDevices("SFC1988", 1, 20);
//            assertNotNull(devices);
//            assertTrue(devices.size()>0);
//        } catch (ApiException ex) {
//            fail(ex.getResponseBody());
//        }
//    }
//
//    @Test
//    public void removeDevice() {
//        try {
////            List<Device> listDevices = service.listDevices();
////            assertNotNull(service.listDevices());
////            Device device = null;
////
////            for (Device listDevice : listDevices) {
////                if (listDevice.getSerialNumber().equals("SFC1988")) {
////                    device = listDevice;
////                    break;
////                }
////            }
//            if (id != null) {
//                Device device = service.getDevice(id);
//                if (device != null) {
//                    assertEquals("SFC1988", device.getSerialNumber());
//                    service.deleteDevice(device.getEntityId());
//                }
//            }
//        } catch (ApiException ex) {
//            fail(ex.getResponseBody());
//        }
//    }
}
